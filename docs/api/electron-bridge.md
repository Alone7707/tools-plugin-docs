...
插件代码统一使用 `window.toolzen` 访问宿主桥接 API。
...
| `fileScan` | `{ paths, options? }` | `Promise<PluginFileScanResult>` | 展开路径条目，返回 `{ ok, entries, errors, truncated }` 信封；对应 `api.file.scan` |
| `fileExists` | `paths: string[]` | `Promise<PluginFileExistsResult>` | 按下标返回存在性（`exists` 数组与入参等长）；对应 `api.file.exists` |
